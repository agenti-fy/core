...
2. **Priority ordering within a repo**: when a poll tick produces multiple eligible routings in the same repo, `dispatchBatch` sorts them by `(method-priority DESC, target_id ASC, persona_name ASC)` before dispatching. Method priority follows the lifecycle stage — `merge > address_review > review > implement > plan` — so in-flight work drains before new work begins, minimising the queue of changes waiting to land (#408). Within the same method, lower GitHub issue/PR numbers (older targets) win the available agent first (`target_id ASC`); `persona_name ASC` is a final deterministic tie-breaker when the same target carries multiple routings (e.g. a PR labelled `agent:skeptic:review` and `agent:scribe:review`). Across repos, dispatches remain parallel — there is no global ordering across the installation.
