...
- Sync SPEC.md §6.4 `JobResult` type with `JobResultSchema` in `packages/shared/src/rpc.ts`: add `final_text?`, `usage_input?`, `usage_output?`, `usage_cache_read?`, `usage_cache_write?`, `cost_usd?`; add `'orphaned'` to the `outcome` union; change `session_id` to `string | null`. Closes #142 (surfaced by #66).
