...
- **Setup wizard encrypt-at-rest** — PEM private keys, OAuth client secrets, and webhook secrets are now encrypted before being written to the wizard checkpoint state file (`~/.config/agentify/setup-<prefix>.json`). New `encryptValue` / `decryptValue` helpers in `packages/setup/src/crypto.ts` (AES-256-GCM + scrypt, no new npm dependencies — uses `node:crypto`). Operators are prompted for a passphrase at wizard entry (`init` twice with confirmation, `resume` once); `AGENTIFY_SETUP_PASSPHRASE` env var bypasses the prompt for headless / CI runs. All `stateForSave` call sites — both the orchestrator saves in `packages/setup/src/index.ts` and the per-persona checkpoint loop in `packages/setup/src/driver/apps.ts` — now encrypt `pem`, `clientSecret`, and `webhookSecret`. `passphrase: string` (required, non-optional) added to `AppsDeps`. Legacy v1 state files are migrated in place on first load. (#484, #485, #486, #487, #488, #489, #491, #492; ADR-001: `docs/adr/001-pem-at-rest-mitigation.md`)
