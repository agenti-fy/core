...
- Treat empty-string env vars for all 12 numeric fields in coordinator config (`packages/coordinator/src/config.ts`) as unset rather than coercing to `0` (which previously crashed the coordinator at startup on `.positive()` / `.min(1)` schema constraints). Affects compose `${VAR-}` expansions where the variable is not set in the environment. Adds `packages/coordinator/src/config.test.ts` with regression coverage. (#283, closes #281.)
